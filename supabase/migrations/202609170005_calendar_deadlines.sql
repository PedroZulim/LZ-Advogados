...
begin
 ...
 if p_id is not null then
  ...
  -- Changing the owner grants the owner the ability to cancel the event. A
  -- non-owner assistant may edit event details, but cannot grant themselves
  -- that capability.
 ...
 if recurrence <> 'none' and nullif(p_data->>'recurrence_until','')::date is null then raise exception 'invalid_recurrence'; end if;
  if p_id is null then
   ...
   -- Lifecycle state is changed only by cancel_event. Generic saves preserve
   -- both status and cancellation timestamp, including for cancelled events.
