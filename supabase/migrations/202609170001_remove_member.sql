-- Membership removal preserves identity references and the audit trail.
alter table public.profiles add column removed_at timestamptz;
alter table public.profiles add constraint removed_members_are_inactive
  check (removed_at is null or not is_active);
drop policy profiles_read_own_organization on public.profiles;
create policy profiles_read_own_organization on public.profiles for select to authenticated
  using (removed_at is null and organization_id = (select private.current_user_org_id()));
...
begin
  ...
  if p_action = 'list' then
    ...
    from (select p.id, p.full_name, p.role, p.is_active, a.email, a.invited_at,
      ...
      where p.organization_id = organization and p.removed_at is null) u;
