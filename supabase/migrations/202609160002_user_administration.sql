-- Only the authenticated Edge Function may invoke privileged operations.
...
create index audit_logs_org_created on public.audit_logs(organization_id, created_at desc);
...
create table private.admin_rate_limits (
...
revoke all on private.admin_rate_limits, private.user_invitations from public, anon, authenticated;

create function public.manage_users(
...
begin
  ...
  if p_action = 'list' then
    ...
    from (select p.id, p.full_name, p.role, p.is_active, a.email, a.invited_at,
      a.email_confirmed_at, a.last_sign_in_at
      from public.profiles p join auth.users a on a.id = p.id
      where p.organization_id = organization) u;
  ...
  elsif p_action in ('change_role', 'set_active', 'revoke_sessions') then
    target_id := (p_payload->>'user_id')::uuid;
    select * into target from public.profiles where id = target_id and organization_id = organization for update;
